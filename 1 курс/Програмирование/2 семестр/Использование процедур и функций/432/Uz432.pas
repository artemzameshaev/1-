...
i,j,x:byte;
 ...
 begin
 writeln(ftxt,'Коэффициенты многочлена:');
for i:=n downto 0 do
write(a[i]:0:1,' ');
writeln(ftxt);
writeln(ftxt,'Значение выражения Р(х+1)-Р(х) при х=1,3,4:');
for x:=1 to 4 do
if x<>2 then
 begin
  write(ftxt,'x=',x,' значение=',P_Calculate(x+1,a)-P_Calculate(x,a):0:1);
  writeln(ftxt);
 end;
 end;
